...
class POMDPWrapper(gym.ObservationWrapper):
    def __init__(self, env_name, pomdp_type='remove_velocity',
        ...
        # self.discrete_act_space = discrete
        ...
        elif pomdp_type == 'remove_position':
            self.remain_obs_idx, self.observation_space = self._remove_position(env_name)
        elif pomdp_type == 'remove_velocity_and_position':
            remain_obs_idx_vel, _ = self._remove_velocity(env_name)
            remain_obs_idx_pos, _ = self._remove_position(env_name)
            self.remain_obs_idx = np.intersect1d(remain_obs_idx_vel, remain_obs_idx_pos)
            self.observation_space = gym.spaces.Box(self.env.observation_space.low[self.remain_obs_idx],
                                                    self.env.observation_space.high[self.remain_obs_idx])
    ...
    def observation(self, obs):
        ...
        if self.pomdp_type == 'remove_velocity' or self.pomdp_type == 'remove_position' \
                or self.pomdp_type == 'remove_velocity_and_position':
    ...
    def _remove_velocity(self, env_name):
        ...
        # Added discrete case in Classic Control
        elif env_name == "CartPole-v1" or env_name == "CartPole-v0":
            remain_obs_idx = list(np.arange(0, 1)) + list(np.arange(2, 4))
        elif env_name == "LunarLander-v2" or env_name == "LunarLander-v1" or env_name == "LunarLander-v0":
            remain_obs_idx = list(np.arange(0, 2)) + list(np.arange(4, 8))
        #  1. MuJoCo
        ...
        obs_high = self.env.observation_space.high[remain_obs_idx]
        obs_low = self.env.observation_space.low[remain_obs_idx]

        observation_space = gym.spaces.Box(obs_low, obs_high)
        return remain_obs_idx, observation_space

    def _remove_position(self, env_name):
        # OpenAIGym
        #  1. Classic Control
        # Added discrete case in Classic Control
        if env_name == "CartPole-v1" or env_name == "CartPole-v0":
            remain_obs_idx = list(np.arange(1, 4))
        elif env_name == "LunarLander-v2" or env_name == "LunarLander-v1" or env_name == "LunarLander-v0":
            remain_obs_idx = list(np.arange(2, 8))
    ...
    def tf_step(self, action):
        # map_convert = partial(tf.convert_to_tensor, dtype=tf.float32)
        map_expand_dims = partial(tf.expand_dims, axis=0)

        next_obs, reward, done = tf.numpy_function(self.env_step, [action], (tf.float32, tf.float32, tf.float32))
        # next_obs, reward, done = map(map_convert, (next_obs, reward, done))
        next_obs, reward, done = map(map_expand_dims, (next_obs, reward, done))
